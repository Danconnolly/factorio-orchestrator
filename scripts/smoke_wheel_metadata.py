...
def main() -> None:
    ...
    if (benchmark_metadata["Name"], benchmark_metadata["Version"]) != ("factorio-benchmark", "0.2.3"):
        raise SystemExit("benchmark wheel metadata is not factorio-benchmark 0.2.3")
    if (orchestrator_metadata["Name"], orchestrator_metadata["Version"]) != ("factorio-orchestrator", "0.2.3"):
        raise SystemExit("orchestrator wheel metadata is not factorio-orchestrator 0.2.3")
    requirements = [requirement.replace(" ", "") for requirement in orchestrator_metadata.get_all("Requires-Dist", [])]
    if "factorio-benchmark==0.2.3" not in requirements:
        raise SystemExit("orchestrator wheel must require factorio-benchmark ==0.2.3")
...
assert importlib.metadata.version("factorio-benchmark") == "0.2.3"
assert importlib.metadata.version("factorio-orchestrator") == "0.2.3"
